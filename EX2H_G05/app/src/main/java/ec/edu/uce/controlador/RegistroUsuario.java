...
public class RegistroUsuario extends AppCompatActivity {
    ...
    private static final Pattern PASSWORD_PATTERN = Pattern.compile("^"+
            "(?=.*[0-9])"+ //almenos un digito
            "(?=.*[a-z])"+ //almenos una minuscula
            "(?=.*[A-Z])"+ //almenos una mayuscula
    ...
    public void registroUsu(){
        ...
        if(!usuarioT.isEmpty() && !claveT.isEmpty() ){
            ...
            if(true){
                ...
                Toast.makeText(getApplicationContext(),"Contraseña no cumple parametros",Toast.LENGTH_SHORT).show();
            }
    ...
    public boolean validarClave(String input){
        if(!PASSWORD_PATTERN.matcher(input).matches()){
            Toast.makeText(getApplicationContext(),"Clave no cumple con los parametros",Toast.LENGTH_SHORT).show();
            return false;
        }else{
            return true;
        }
